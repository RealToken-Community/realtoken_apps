...
class _UpdatesPageState extends State<UpdatesPage> {
          ...
          child: Builder(
            builder: (context) {
              ...
              List<Map<String, dynamic>> recentChanges =
                  dataManager.getRecentTokenChanges(
                      days: showAllChanges
                          ? null
                          : 365, // null = pas de limite, sinon 1 an par défaut
              ...
              if (showUserTokensOnly) {
                Set<String> userTokenUuids = dataManager.portfolio
                    .map((token) =>
                        (token['uuid']?.toLowerCase() ?? '') as String)
                    .toSet();

                recentChanges = recentChanges
                    .where((change) => userTokenUuids
                        .contains(change['token_uuid']?.toLowerCase() ?? ''))
                    .toList();
              }
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            List<Map<String, dynamic>> changesForDate =
                                changesByDate[date]!;
  ...
  List<Widget> _buildChangesForDateWithTimeline(
    ...
    for (int tokenIndex = 0; tokenIndex < tokenKeys.length; tokenIndex++) {
      ...
      if (tokenChanges.isNotEmpty) {
        ...
        Map<String, dynamic>? fullTokenInfo =
            dataManager.allTokens.cast<Map<String, dynamic>?>().firstWhere(
                  (token) =>
                      token?['uuid']?.toLowerCase() == tokenUuid.toLowerCase(),
                  orElse: () => null,
                );
  ...
  Widget _buildTimelineIndicator(BuildContext context, int dateIndex,
      int tokenIndex, int totalDates, int tokensInDate, Color changeColor) {
            ...
            children: [
              ...
              if (!isVeryFirstItem)
                Expanded(
                  child: Transform.translate(
                    offset:
                        Offset(0, -8), // Remonte dans l'espace entre les cartes
                    child: Container(
                      width: 2,
              ...
              if (!isVeryLastItem)
                Expanded(
                  child: Transform.translate(
                    offset:
                        Offset(0, 8), // Descend dans l'espace entre les cartes
                    child: Container(
                      width: 2,
                      height: double.infinity,
                      decoration: BoxDecoration(
  ...
  Widget _buildChangeItem(
      BuildContext context, Map<String, dynamic> change, AppState appState) {
